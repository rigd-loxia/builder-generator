...
abstract class CompilingExtension implements BeforeEachCallback {
    ...
    /**
     * Build the default test compilation classpath needed for compiling the generated sources once the processor has run.
     */
    private static List<String> buildTestCompilationClasspath() {
        Collection<String> whitelist = Arrays.asList(
            ...
            "annotations" + File.separator + "target",
            // MapStruct annotations in single module build
            "nl" + File.separator + "loxia" + File.separator + "bibliotheek" + File.separator + "apt" + File.separator
    ...
    /**
     * Build the annotation processor classpath. i.e. the classpath that is needed to run the annotation processor with its own
     * dependencies only. The optional dependencies are not needed in this classpath.
     */
    private static List<String> buildProcessorClasspath() {
        Collection<String> whitelist = Arrays.asList(
            "generator" + File.separator + "target", // the processor itself,
            "freemarker",
            "gem-api");
    ...
    private void assertResult(CompilationOutcomeDescriptor actualResult, ExtensionContext context) throws Exception {
        ...
        if (expectedResult.getCompilationResult() == CompilationResult.SUCCEEDED) {
            assertThat(actualResult.getCompilationResult()).describedAs(
                "Compilation failed. Diagnostics: " + actualResult.getDiagnostics()).isEqualTo(
     ...
     *
     * @return A map containing the package were to look for a resource (key) and the resource (value) to be compiled for this test
     */
    ...
    protected Object loadAndInstantiate(ClassLoader processorClassloader, Class<?> clazz) {
        try {
            return processorClassloader.loadClass(clazz.getName()).getConstructor().newInstance();
        }
        catch (Exception e) {
