...
import nl.loxia.builder.generator.ap.BuilderProcessor;
...
class JdkCompilingExtension extends CompilingExtension {
    ...
    private static final ClassLoader DEFAULT_PROCESSOR_CLASSLOADER =
        new ModifiableURLClassLoader(new FilteringParentClassLoader(/* "nl.loxia." */ "nl.loxia.builder.generator.ap.test."))
    ...
    protected CompilationOutcomeDescriptor compileWithSpecificCompiler(CompilationRequest compilationRequest,
        ...
        if (additionalCompilerClasspath == null) {
            ...
            processorClassloader = new ModifiableURLClassLoader(
                new FilteringParentClassLoader("nl.loxia."))
                    .withPaths(PROCESSOR_CLASSPATH)
        ...
        task.setProcessors(
            Arrays.asList((Processor) loadAndInstantiate(processorClassloader, BuilderProcessor.class)));
    ...
    /**
     * The JDK 8 compiler needs some special treatment for the diagnostics. See comment in the function.
     */
    @Override
